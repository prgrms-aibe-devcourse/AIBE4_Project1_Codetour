...
app.use((req, res, next) => {
  res.setHeader(
    ...
    [
      ...
      "script-src 'self' 'unsafe-inline' 'unsafe-eval' https: https://esm.sh https://cdn.jsdelivr.net",
...
// /source/pages/** 정적 제공
app.use("/source/pages",       express.static(path.join(__dirname, "../source/pages")));
app.use("/source/pages/index", express.static(path.join(__dirname, "../source/pages/index")));
app.use("/source/pages/map",   express.static(path.join(__dirname, "../source/pages/map")));

// 메인 페이지
app.get("/", (_req, res) => {
  if (!fs.existsSync(indexPath)) return res.status(404).send("index.html을 찾을 수 없습니다");
  res.sendFile(indexPath);
});

// (옵션) 간단한 /map 라우트
app.get("/map", (_req, res) => {
  if (!fs.existsSync(mapPath)) return res.status(404).send("map_page.html을 찾을 수 없습니다");
...
app.get("/api/search-by-content", async (req, res) => {
  ...
  if (!contentName) return res.status(400).json({ error: "콘텐츠 이름이 필요합니다." });
  ...
  try {
    ...
    const locationNames = [...new Set(contentRows.map((r) => r.name).filter(Boolean))];
...
app.get("/api/route", async (req, res) => {
  ...
  try {
    const resp = await axios.get("https://apis-navi.kakaomobility.com/v1/directions", {
      params: {
...
app.get("/api/course/:userId", async (req, res) => {
  try {
    const { data, error } = await supabase
      .from("courses")
      .select("id, name, course_places(place_name, lat, lng, order_index, place_id, media_title)")
...
app.delete("/api/course/:courseId", async (req, res) => {
  try {
    const { error } = await supabase.from("courses").delete().eq("id", req.params.courseId);
    if (error) throw error;
